...
public class TrapReceiver implements CommandResponder {
	...
	public static void main(String[] args)
	{
		...
		try
		{
			snmp4jTrapReceiver.listen(new UdpAddress("127.0.0.1/2222"));
		}
		catch (IOException e)
		{
		...
		//	System.out.println("!!!!!!!!!!!! " + pdu.getVariableBindings().get(0).getOid() + "      " +  pdu.getVariableBindings().get(0).getVariable());
			...
			if(pduType==PDU.SET){
				try {
					...
					TestSNMPAgent client = new TestSNMPAgent("udp:127.0.0.1/2222");
					...
					for(int i=0;i<pdu.getVariableBindings().size();i++) {
						...
						System.out.println("dsssssssssssssssssssssssssssss   "+ pdu.getVariableBindings().get(i).getVariable().getSyntaxString());
					}
